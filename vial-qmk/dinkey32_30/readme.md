...
Vial keyboard definition for the Dinkey 32|30 wired build (Pro Micro / ATmega32U4).
...
See [`vial-qmk/README.md`](../README.md) for setup, compile, and flash instructions.
...
```bash
make idlebuilds/dinkey32_30:vial:flash
```
...
The 32|30 PCB supports one or two switches in the 4th pinky column. The matrix includes both positions. Unpopulated positions register no keypress — no firmware changes are needed when changing key count.

## Vial

Once flashed, open the [Vial app](https://get.vial.today), connect the left half via USB, and the keyboard will be detected automatically. Remap keys in real time — no reflash required.
